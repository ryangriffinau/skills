...
assert_contains "$output" "Readiness note: if Codex panes are dead shells, do not use ntm respawn" "readiness recovery note"
assert_contains "$output" "ntm kill 'alpha' --force" "fresh-spawn kill command"
...
assert_contains "$output" "ntm kill 'team session' --force" "fresh-spawn quoted kill command"
assert_not_contains "$output" "ntm respawn 'team session'" "no hollow respawn command"
