...
assert_file_contains "$repo/.gitattributes" ".beads/issues.jsonl merge=union" "beads union merge attribute"
assert_file_executable "$repo/scripts/ci/flywheel-ready.sh" "clone-owned ready helper copied"
assert_output_contains "$SETUP_OUT" ".gitattributes: .beads/issues.jsonl merge=union" "union attribute reported"
run_setup "$repo"
assert_output_contains "$SETUP_OUT" "already merges beads by union" "union attribute idempotent"
[ "$(grep -c 'merge=union' "$repo/.gitattributes")" -eq 1 ] || fail "union attribute must not be duplicated on re-run"
