...
| `setup [path]` | `link` + `br init` (+ a starter **verification bead**) + `ntm init` + lease guard + `.flywheel/profile` + beads union merge (`.gitattributes`) + `scripts/ci/flywheel-ready.sh` (clone-owned ready beads), check AGENTS.md, then run `verify` |
