...
argument-hint: "[cod-panes] [cc-panes] [tick-interval] — defaults: 3 cod (GPT-6 Sol, medium), 1 cc (Opus 5, medium), 5m"
---

Arguments (if given): the number of cod panes, the number of cc panes, and the loop tick interval. If absent, default to 3 cod panes (GPT-6 Sol, medium thinking effort), 1 cc pane (Opus 5, medium thinking effort), and a 5-minute tick — the configuration described below.
...
**DO NOT use your subagents for doing any work on beads. You must use the prescribed ntm panes to spawn agents for completing work.** This covers implementation, review, verification and evidence gathering — every step that touches a bead. A subagent is invisible to tmux, to Agent Mail and to `ntm --robot-context`, so its work cannot be observed, restarted, rate-limited or handed off like a pane's, and the conductor loses the single consistent process it exists to run. If the panes are busy or out of quota, queue the work as a brief and wait for a pane; do not route around the swarm.

**Multi-person beads (every tick).** The shared `.beads/issues.jsonl` carries every teammate's beads, and bare `br ready` / `bv --robot-triage` list all of them. Dispatch **only** from `scripts/ci/flywheel-ready.sh` (installed by `/flywheel-local-launcher` setup): it keeps the beads created in this clone — by you or your agents — using the `source_repo_path` that `br` stamps on every bead. Use `bv` to rank and inspect, never to choose what to send; every nudge, marching order and "claim the top bead" message names the helper, not `br ready`. `scripts/ci/flywheel-ready.sh --foreign` shows teammates' ready beads for awareness only. If setup has not run (no `.gitattributes` union line or no helper), run `/flywheel-local-launcher` setup before the first dispatch. When any `br` call fails with `Duplicate issue id` or `ID collision` after a pull, a teammate minted the same child id: run `br sync --merge`; if this `br` build refuses (pre-#512), hand the exact line to the user and pause dispatch — never delete a line yourself.

**Pane context hygiene (every tick).** An agent pane re-sends its whole conversation on every step, so long conversations cost far more than short ones. Keep one bead per conversation where possible. Between beads — never mid-bead — read each pane's context (`ntm --robot-context <session>`, or the agent's own session log for the exact number) and restart any pane at or above ~40% of its window or after two auto-compactions: recover its Agent Mail identity, relaunch it, and re-dispatch a bead-scoped brief. cod restarts with `/exit` and the same launch line; cc restarts with `/clear`. Every brief asks for quiet test reporters and `tail`, and forbids whole-file and whole-ledger dumps. Done when no pane starts a bead above the threshold.
